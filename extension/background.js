...
// Default config (overridden by chrome.storage.local in split-VPS mode)
let _config = {
...
function connectToAgent() {
  ...
  try {
    // Use configured WS URL (supports split-VPS mode)
    ws = new WebSocket(_config.wsUrl);
  } catch (e) {
  ...
  ws.onopen = () => {
    console.log('[FlowAgent] Connected to agent at', _config.wsUrl);
    ...
    // Send auth token if configured (for split-VPS security)
    if (_config.authToken) {
      ws.send(JSON.stringify({ type: 'auth', token: _config.authToken }));
    }
  ...
  ws.onmessage = async ({ data }) => {
    try {
      ...
      if (msg.method === 'api_request') {
        ...
        // keepalive response
      } else if (msg.method === 'inject_cookies') {
        await handleInjectCookies(msg);
      }
    } catch (e) {
...
function sendToAgent(msg) {
  ...
  if (msg.id) {
    // Use configured HTTP callback URL (supports split-VPS mode)
    fetch(_config.httpCallbackUrl, {
      method: 'POST',
...
async function findFlowTab(timeoutMs = 10000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const tabs = await chrome.tabs.query({});
    const flowTab = tabs.find((tab) => {
      const url = tab.pendingUrl || tab.url || '';
      return url.startsWith('https://labs.google/fx/tools/flow') ||
        /^https:\/\/labs\.google\/fx\/[^/]+\/tools\/flow/.test(url);
    });
...
async function solveCaptcha(requestId, captchaAction) {
  let flowTab = await findFlowTab();

  if (!flowTab) {
    // Auto-open Flow tab and wait briefly before returning error
    try {
      await chrome.tabs.create({ url: 'https://labs.google/fx/tools/flow', active: false });
      flowTab = await findFlowTab(15000);
      if (!flowTab) return { error: 'NO_FLOW_TAB' };
...
async function handleInjectCookies(msg) {
  const { id, params } = msg;
  const { cookies, site } = params;

  if (!cookies || !Array.isArray(cookies)) {
    sendToAgent({ id, error: 'MISSING_COOKIES', result: { success: false, error: 'cookies must be an array' } });
    return;
  }

  console.log(`[FlowAgent] Injecting ${cookies.length} cookies for site: ${site}`);
  const results = [];
  let allSuccess = true;
  ...
  for (const cookie of cookies) {
    try {
      const cookieData = {
        url: cookie.url || `https://${cookie.domain || site}/`,
        name: cookie.name,
        value: cookie.value,
      };
      ...
      if (cookie.sameSite) cookieData.sameSite = cookie.sameSite;
      ...
      console.error(`[FlowAgent] Failed to inject cookie '${cookie.name}':`, e);
...
async function handleTrpcRequest(msg) {
  ...
  const trpcFlowKey = (params && params.flowKey) || flowKey;
  if (trpcFlowKey) {
    fetchHeaders['authorization'] = `Bearer ${trpcFlowKey}`;
  }
...
async function handleApiRequest(msg) {
  ...
  try {
    ...
    // Step 3: Use flowKey for auth — prefer agent-provided key (for multi-account routing)
    const activeFlowKey = (params && params.flowKey) || flowKey;
    if (!activeFlowKey) {
