...
_COLUMNS = {
    ...
    "request": {"status", "request_id", "media_id", "output_url", "error_message", "retry_count", "next_retry_at", "source_media_id", "payload_json", "progress_pct", "progress_stage", "account_id", "project_id", "chrome_pid", "updated_at"},
}
...
async def create_request(req_type: str, orientation: str = None,
                         ...
                         payload_json: str = None, **_kw) -> dict:
    db = await get_db()
    rid, now = _uuid(), _now()
    async with _db_lock:
        await db.execute(
            """INSERT INTO request (id,project_id,video_id,scene_id,character_id,type,orientation,source_media_id,payload_json,created_at,updated_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (rid, project_id, video_id, scene_id, character_id, req_type, orientation,
             source_media_id, payload_json, now, now))
        await db.commit()
...
async def update_request_progress(rid: str, pct: int, stage: str = "") -> None:
    """Update progress_pct and progress_stage on a request."""
    db = await get_db()
    async with _db_lock:
        await db.execute(
            "UPDATE request SET progress_pct=?, progress_stage=?, updated_at=? WHERE id=?",
            (pct, stage, _now(), rid))
        await db.commit()
...
async def list_requests(scene_id: str = None, status: str = None,
    ...
    if from_date:
        q += " AND created_at >= ?"; params.append(from_date)
    if to_date:
        if "T" in to_date:
            q += " AND created_at <= ?"; params.append(to_date)
        else:
            q += " AND created_at <= ?"; params.append(to_date + "T23:59:59Z")
