...
CREATE TABLE IF NOT EXISTS request (
    ...
    payload_json  TEXT,    -- direct API request payload for queued operations
    created_at    TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
    updated_at    TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))
);
...
CREATE INDEX IF NOT EXISTS idx_account_site ON account(site);
CREATE INDEX IF NOT EXISTS idx_account_status ON account(status);

CREATE TABLE IF NOT EXISTS chrome_profile (
    id          TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_chrome_profile_account ON chrome_profile(account_id);
CREATE INDEX IF NOT EXISTS idx_chrome_profile_status ON chrome_profile(status);
...
async def init_db():
    ...
    async with aiosqlite.connect(str(DB_PATH)) as db:
        ...
        if "progress_pct" not in req_columns:
            await db.execute("ALTER TABLE request ADD COLUMN progress_pct INTEGER DEFAULT 0")
            logger.info("Migrated: added progress_pct column to request table")
        if "progress_stage" not in req_columns:
            await db.execute("ALTER TABLE request ADD COLUMN progress_stage TEXT DEFAULT ''")
            logger.info("Migrated: added progress_stage column to request table")
        ...
        if "account_id" not in request_columns:
            await db.execute("ALTER TABLE request ADD COLUMN account_id TEXT")
            logger.info("Migrated: added account_id column to request table")
        if "chrome_pid" not in request_columns:
            await db.execute("ALTER TABLE request ADD COLUMN chrome_pid INTEGER")
            logger.info("Migrated: added chrome_pid column to request table")
...
""")
    ...
    created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')))""")
        ...
        # Migration: create account table if missing
        cursor = await db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='account'")
        if not await cursor.fetchone():
            await db.execute("""CREATE TABLE account (
    id TEXT PRIMARY KEY, site TEXT NOT NULL, name TEXT NOT NULL,
    cookies TEXT NOT NULL DEFAULT '[]', models TEXT NOT NULL DEFAULT '[]',
    max_count INTEGER NOT NULL DEFAULT 1, in_use INTEGER NOT NULL DEFAULT 0,
    locked INTEGER NOT NULL DEFAULT 0, locked_at TEXT,
    status TEXT NOT NULL DEFAULT 'ACTIVE' CHECK(status IN ('ACTIVE','DISABLED','LOCKED')),
    project_id TEXT REFERENCES project(id),
    created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
    updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')))""")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_account_site ON account(site)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_account_status ON account(status)")
            logger.info("Migrated: created account table")
        # Migration: create chrome_profile table if missing
        cursor = await db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='chrome_profile'")
        if not await cursor.fetchone():
            await db.execute("""CREATE TABLE chrome_profile (
    id TEXT PRIMARY KEY, account_id TEXT NOT NULL REFERENCES account(id) ON DELETE CASCADE,
    site TEXT NOT NULL, profile_dir TEXT NOT NULL, pid INTEGER,
    status TEXT NOT NULL DEFAULT 'ACTIVE' CHECK(status IN ('ACTIVE','CLOSING','CLOSED')),
    created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
    closed_at TEXT)""")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_chrome_profile_account ON chrome_profile(account_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_chrome_profile_status ON chrome_profile(status)")
            logger.info("Migrated: created chrome_profile table")
        # Migration: create default_model table + seed defaults
        cursor = await db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='default_model'")
        if not await cursor.fetchone():
            await db.execute("""CREATE TABLE default_model (
    operation_type TEXT PRIMARY KEY,
    model_value TEXT NOT NULL,
    updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')))""")
            logger.info("Migrated: created default_model table")
        # Seed defaults if table is empty
        cursor = await db.execute("SELECT COUNT(*) FROM default_model")
        ...
        if count == 0:
            ...
            await db.executemany(
                "INSERT OR IGNORE INTO default_model (operation_type, model_value) VALUES (?, ?)",
                defaults,
            )
            logger.info("Seeded %d default models", len(defaults))
        # Migration: add project_id to account table
        cursor = await db.execute("PRAGMA table_info(account)")
        columns = [row[1] for row in await cursor.fetchall()]
        if "project_id" not in columns:
            await db.execute("ALTER TABLE account ADD COLUMN project_id TEXT REFERENCES project(id)")
            logger.info("Migrated: added project_id column to account table")
        await db.commit()
