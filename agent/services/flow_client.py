...
class FlowClient:
    ...
    Multi-account support: stores ALL extension connections (one per Chrome profile)
    and routes API commands to the correct extension based on account flow keys.
    """

    def __init__(self):
        self._extension_ws_set: set = set()  # All connected extension WebSockets
        self._pending: dict[str, asyncio.Future] = {}
        self._flow_key: Optional[str] = None  # Most recently captured key (backward compat)
        self._flow_key_ws: dict[str, object] = {}  # flowKey → WebSocket that captured it
        self._account_flow_keys: dict[str, str] = {}  # account_id → flowKey
        self._account_flow_key_timestamps: dict[str, float] = {}  # account_id → timestamp when key was captured
        self._current_account_id: Optional[str] = None  # Set by _process_one for _send routing
        # WS stats
    ...
    def set_extension(self, ws):
        """Called when extension connects via WS. Adds to set (no overwrite)."""
        self._extension_ws_set.add(ws)
        self._ws_connect_count += 1
        self._ws_connected_at = time.time()
        logger.info("Extension connected #%d (total=%d, waiting for token_captured)",
                     self._ws_connect_count, len(self._extension_ws_set))

    def clear_extension(self, ws=None):
        """Called when extension disconnects. Removes specific ws or all."""
        if ws:
            self._extension_ws_set.discard(ws)
            # Clean up flow key → ws mappings for this ws
            self._flow_key_ws = {k: v for k, v in self._flow_key_ws.items() if v is not ws}
            # Clear account flow keys that mapped to this ws
            stale_keys = [k for k, v in self._account_flow_keys.items()
                          if self._flow_key_ws.get(v) is None and v not in self._flow_key_ws]
            for k in stale_keys:
                del self._account_flow_keys[k]
                self._account_flow_key_timestamps.pop(k, None)
        else:
            self._extension_ws_set.clear()
            self._flow_key_ws.clear()
            self._account_flow_keys.clear()
            self._account_flow_key_timestamps.clear()
        self._ws_disconnect_count += 1
        self._ws_last_disconnect_at = time.time()
        # Cancel pending futures for requests that were sent to this ws
        if ws:
            pending_to_cancel = []
            for req_id, future in list(self._pending.items()):
                if not future.done():
                    # Cancel all — we can't easily tell which ws a pending req is for
                    pending_to_cancel.append((req_id, future))
            for req_id, future in pending_to_cancel:
                future.set_exception(ConnectionError("Extension disconnected"))
            if pending_to_cancel:
                logger.warning("Extension disconnected, cancelled %d pending requests", len(pending_to_cancel))
        else:
            ...
            for req_id, future in pending_copy:
                if not future.done():
                    future.set_exception(ConnectionError("Extension disconnected"))
            self._pending.clear()
        logger.warning("Extension disconnected (remaining=%d)", len(self._extension_ws_set))
    ...
    def connected(self) -> bool:
        return len(self._extension_ws_set) > 0

    @property
    def connected_count(self) -> int:
        return len(self._extension_ws_set)

    def get_ws_for_flow_key(self, flow_key: str):
        """Get the WebSocket that captured a specific flow key."""
        return self._flow_key_ws.get(flow_key)

    def get_account_flow_key(self, account_id: str) -> Optional[str]:
        """Get the flow key stored for a specific account."""
        return self._account_flow_keys.get(account_id)

    def store_account_flow_key(self, account_id: str, flow_key: str):
        """Store the flow key captured for a specific account with timestamp."""
        import time
        self._account_flow_keys[account_id] = flow_key
        self._account_flow_key_timestamps[account_id] = time.time()
    ...
    async def handle_message(self, data: dict, ws=None):
        ...
        Args:
            data: Parsed JSON message from extension.
            ws: The WebSocket that sent this message (for routing).
        """
        if data.get("type") == "token_captured":
            new_key = data.get("flowKey")
            changed = bool(new_key and new_key != self._flow_key)
            self._flow_key = new_key
            # Map this flowKey to the WebSocket that sent it
            if new_key and ws:
                self._flow_key_ws[new_key] = ws
            if changed:
                logger.info("Flow key captured from extension (ws=%s)", id(ws) if ws else "?")
        ...
        if data.get("type") == "ping":
            # Respond to keepalive on the same ws
            if ws:
                await ws.send(json.dumps({"type": "pong"}))
            return

        # Response to a pending request (comes via HTTP callback, not WS)
        req_id = data.get("id")
    ...
    async def _send(self, method: str, params: dict, timeout: float = 300,
        ...
        Routes to the correct Chrome extension based on account_id:
        1. Looks up account's flow key from _account_flow_keys
        2. Finds the WebSocket that captured that flow key
        3. Sends command with flowKey in params so extension uses correct token
        ...
        """
        # Find the right WebSocket for this account
        ws = None
        flow_key = None
        # Use explicit account_id, or fall back to context set by _process_one
        effective_account = account_id or getattr(self, '_current_account_id', None)
        if effective_account:
            flow_key = self._account_flow_keys.get(effective_account)
            if flow_key:
                ws = self._flow_key_ws.get(flow_key)
        # Fallback: use any connected extension
        if not ws:
            ws = next(iter(self._extension_ws_set), None) if self._extension_ws_set else None
        ...
        try:
            # Include flowKey in params so extension uses correct token
            send_params = dict(params)
            if flow_key:
                send_params["flowKey"] = flow_key
    ...
    async def generate_images(self, prompt: str, project_id: str,
        ...
        # Resolve model: short name (NANO_BANANA_2) -> API value (NARWHAL)
        if model_name:
            resolved_model = IMAGE_MODELS.get(model_name, model_name)
        else:
            resolved_model = IMAGE_MODELS.get("NANO_BANANA_2", "NARWHAL")
    ...
    async def generate_video(self, start_image_media_id: str, prompt: str,
        ...
        Args:
            video_model_key: Direct model key (e.g. "veo_3_1_i2v_lite_low_priority").
                If provided, skips tier-based lookup.
        """
        if video_model_key:
        ...
        if not model_key:
            return {"error": f"No model for tier={user_paygate_tier} ratio={aspect_ratio}"}
    ...
    async def generate_video_from_references(self, reference_media_ids: list[str],
        ...
        Args:
            ...
            video_model_key: Direct model key. If provided, skips tier-based lookup.
        """
        if video_model_key:
            model_key = video_model_key
        else:
