...
async def list_all(status: str = None, account_id: str = None):
    repo = _get_repo()
    if account_id:
        from agent.db.schema import get_db
        db = await get_db()
        cur = await db.execute(
            "SELECT p.* FROM project p JOIN account a ON a.project_id=p.id WHERE a.id=?",
            (account_id,))
        rows = await cur.fetchall()
        return [repo._row_to_project(dict(r)) for r in rows]
