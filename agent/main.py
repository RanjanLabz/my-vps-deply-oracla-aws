...
from agent.api.defaults import router as defaults_router
...
from agent.services.chrome_manager import get_chrome_manager
...
async def ws_handler(websocket):
    ...
    logger.info("Extension connected from %s (total=%d)", websocket.remote_address, client.connected_count)
    ...
    try:
        async for raw in websocket:
            try:
                data = json.loads(raw)
                await client.handle_message(data, ws=websocket)
        ...
        logger.info("Extension disconnected (remaining=%d)", client.connected_count)
...
async def lifespan(app: FastAPI):
    ...
    # Crash recovery: unlock stale accounts
    from agent.api.accounts import recover_stale_locks, recover_stuck_in_use
    recovered = await recover_stale_locks()
    if recovered:
        logger.info("Crash recovery: unlocked %d stale accounts", recovered)

    # Crash recovery: reset stuck in_use counters (accounts with no active Chrome sessions)
    stuck_recovered = await recover_stuck_in_use()
    if stuck_recovered:
        logger.info("Crash recovery: reset in_use for %d stuck accounts", stuck_recovered)

    # Crash recovery: adopt orphaned Chrome profiles (reuse instead of killing)
    from agent.db.schema import get_db, _db_lock
    db = await get_db()
    async with _db_lock:
        cursor = await db.execute("SELECT id, account_id, profile_dir, pid, site FROM chrome_profile WHERE status='ACTIVE'")
        orphaned = await cursor.fetchall()
    cdp = get_cdp_client()
    for row in orphaned:
        profile_id, account_id, profile_dir, pid, site = row["id"], row["account_id"], row["profile_dir"], row["pid"], row["site"]
        if pid:
            try:
                import psutil
                ...
                alive = False
            if alive:
                # Adopt into _sessions so the existing Chrome can be reused
                from agent.services.cdp_client import CDPSession
                session = CDPSession(
                    session_id=f"orphan_{profile_id[:8]}",
                    account_id=account_id or "default",
                    site=site or "labs.google",
                    profile_dir=profile_dir,
                )
                session.pid = pid
                session.status = "RUNNING"
                cdp._sessions[session.session_id] = session
                cdp._start_idle_timer(session.session_id)
                logger.info("Crash recovery: adopted orphaned Chrome pid=%d for account %s", pid, (account_id or "default")[:8])
            else:
                # Process dead — mark as closed
                from datetime import datetime, timezone
                now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
                async with _db_lock:
                    await db.execute("UPDATE chrome_profile SET status='CLOSED', closed_at=? WHERE id=?", (now, profile_id))
                    await db.commit()
                logger.info("Crash recovery: closed dead Chrome profile %s (pid=%d)", profile_id[:8], pid)
    ...
    # Cleanup: close all Chrome profiles and Redis
    chrome_mgr = get_chrome_manager()
    await chrome_mgr.close_all()
